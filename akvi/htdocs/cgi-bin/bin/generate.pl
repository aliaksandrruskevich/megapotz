...
foreach my $dir (keys(%index)) {
    ...
    $num++;
        ...
        if ($dir eq '/') {
            $menu_file = "../tmpl/col_index.tmpl";
        }
        elsif ($dir eq '/price/') {
            $menu_file = "../tmpl/col_price.tmpl";
        }
        elsif ( ($dir eq '/abonentskoe/') || ($dir eq '/vosstanovlenie/') || ($dir eq '/virus/') || ($dir eq '/programs/') || ($dir eq '/windows/') || ($dir eq '/sborka/') || ($dir eq '/ustanovka/') || ($dir eq '/wifi/') ) {
            $menu_file = "../tmpl/col_textpage.tmpl";
        }
        else {
            $menu_file = "../tmpl/col_other.tmpl";
        }
...
open (XML_SITEMAP, ">../../www/sitemap.xml") or die "Can't open file: $!";
