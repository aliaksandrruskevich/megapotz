...
//	}
	...
	function index_page() {
	  ...
	  $criteria1->order= "t.sort, t.date";
	  $criteria1->condition="(t.status=0 OR t.status is NULL) AND (t.type=1 OR t.type=2)";
	  $process=Objects::model()->findAll($criteria1);
	  
	  $this->save_to_file('index.html',$this->renderPartial('index',array('objects'=>$objects, 'process'=>$process),true));
	  return " успешно <br>";
	}
